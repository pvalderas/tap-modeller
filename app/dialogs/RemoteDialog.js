...
import {showLinkPanel,updateMenuTitle,showLinkedLabel} from "../menu/Menu.js";
...
export default class UploadDialog extends React.Component {
  ...
  loadBPMN(compo){
    ...
    fetch(url+"wot/iotbps/"+user+"/"+compo.id)
      ...
      .then(compoBPMN => {
         ...
         sessionStorage.setItem("compoName",compo.id);
         showLinkPanel();
  ...
  loadCompos(){
     ...
     let user=this.state.userID;
      ...
      fetch(url+"wot/iotbps/user/"+user)
        .then(function (response) {
